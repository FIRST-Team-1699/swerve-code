...
public class SwerveUpdateThread extends Thread {
	...
	public void run() {
		// This infinite while loop keeps the thread from dying. It might need
		// to be changed so that the
		// run() method is called when the robot is enabled.
		while (true) {

			// Right now, this updates all the modules in one order. This should
			...
			while (ds.isEnabled()) {
				...
				// Sleepy time
				try {
					Thread.sleep(50); // again, this value should be adjusted so
										// that that the robot isn't
										// trying to update PID 1000s of times
										// per second
			...
			// To keep the thread from checking if the robot is enabled 1000s of
			// times per second
			try {
				Thread.sleep(100);
			} catch (InterruptedException e) {
